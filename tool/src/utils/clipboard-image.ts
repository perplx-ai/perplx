...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
  const tmpFile = join(tmpdir(), `perplx-wsl-clip-${randomUUID()}.png`);
  ...
  try {
    ...
    const psScript = [
      ...
      '$path = $env:PERPLX_WSL_CLIPBOARD_IMAGE_PATH',
    ...
    const result = runCommand('powershell.exe', ['-NoProfile', '-Command', psScript], {
      ...
      env: { ...process.env, PERPLX_WSL_CLIPBOARD_IMAGE_PATH: winPath }
    });
    if (!result.ok) {
      return null;
    }
...
export async function readClipboardImage(options?: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform }): Promise<ClipboardImage | null> {
