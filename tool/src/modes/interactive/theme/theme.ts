...
import { isBunBinary, getCustomThemesDir, getThemesDir } from '../../../config.js';

import embeddedDarkTheme from './dark.json' with { type: 'json' };
import embeddedLightTheme from './light.json' with { type: 'json' };
...
function resolveVarRefs(value: ColorValue, vars: Record<string, ColorValue>, visited = new Set<string>()): string | number {
...
function getBuiltinThemes(): Record<string, ThemeJson> {
  if (!BUILTIN_THEMES) {
    if (isBunBinary) {
      BUILTIN_THEMES = {
        dark: embeddedDarkTheme as unknown as ThemeJson,
        light: embeddedLightTheme as unknown as ThemeJson
      };
    } else {
...
function createTheme(themeJson: ThemeJson, mode?: ColorMode, sourcePath?: string): Theme {
  ...
  const bgColorKeys: Set<string> = new Set(['selectedBg', 'userMessageBg', 'customMessageBg', 'toolPendingBg', 'toolSuccessBg', 'toolErrorBg']);
...
const THEME_KEY = Symbol.for('perplexity-code:theme');
